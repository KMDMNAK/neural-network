...
class NeuralNetWork:
    ...
    def train(self,epoch,batch_size,nor_loc=None):
        ...
        for w in range(epoch):
            ...
            for i in range(count):
                self.forward_excute(self.variables[batch_size*i:batch_size*(i+1)])
                self.back_propagation(self.teachers[batch_size*i:batch_size*(i+1)])
    ...
    def back_delta(self,index,delta_next):
        ...
        delta=self.layers[index].dactivater(self.layers[index].input)*np.dot(delta_next,self.weights[index])
        return delta
